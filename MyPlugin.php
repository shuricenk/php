...
class MyPlugin {
    ...
    public function __construct(){
        ...
        register_activation_hook( __FILE__, array($this,'install') );//хук на установку своего option in db
        register_uninstall_hook(__FILE__,array($this,'uninstall'));//регистрируем хук на делит своей опции из бд
        //register_deactivation_hook( __FILE__, array($this,'uninstall') ); действия при диактивации плагина
    }

    public function init(){
        add_shortcode('hi', array($this,'hi_word'));
        add_action('admin_menu',array($this,'AddMenuPage')); //в админ-меню включаем вкладку плагина

    }
    public function hi_word() {            
                echo 'THIS HELL WORDs!';  
    }
	public function install(){
        add_option('my_test_option','255.255.255.255','','no'); //добавление опции в базу -> name,value,'none',автоподгрузка из базы
    }
...
//            $page = $_REQUEST['page']; var_dump($page); //проверочки
    ...
    public function scripts_styles(){
        wp_enqueue_script( 'uilang', __DIR__. '/js/uilang.js','jQuery','',true);
...
$tag = $_REQUEST['tag_sh'];
$content = $_REQUEST['content_sh'];
    
    $use = new MyPlugin();
    $use->init();
    

// $use->test_sh($tag,$content);
//не передает опции в базу при активации?
